...
@push('title', __('front.medical questions'))
@push('styles')
            ...
            <div class="col-md-12">
                <ul class="nav nav-tabs custom_tabs" id="myTab" role="tablist">
                    <li class="nav-item me-2">
                        <a href="{{ route('blogs') }}"
                           class="btn btn-primary not-active">{{__('front.Medical topics')}}</a>
                    </li>
                    <li class="nav-item me-2">
                        <a style="background-color: #19B9B7; border:1px solid #19B9B7"
                           href="{{ route('blogQuestions') }}"
                           class="btn btn-primary active">{{__('front.medical questions')}}</a>
                    </li>
                    <li class="nav-item me-2">
                        <a href="{{ route('blog_public_questions') }}"
                           class="btn btn-primary not-active">{{__('front.Public Questions')}}</a>
                    </li>
                </ul>
            </div>
        </div>
                    ...
                    <div class="sidebar">
                        <div class="card border-0">
                            <ul class="generic-list-item generic-list-item-highlight fs-15">
                                <li class="lh-26 active"><a href="{{route('blogQuestions')}}"><i
                                            class="la la-home mr-1 text-black"></i><span
                                            class="badge badge-primary">{{$Questions->total()}}</span> {{__('front.All')}}
                                    </a></li>
                                @forelse($Categories as $Category)
                                    <li class="lh-26"><a href="{{route('blogquestionCategory' , $Category->id )}}"><i
                                                class="la la-home mr-1 text-black"></i> <span
                ...
                <div class="col-lg-9">
                    ...
                    <div class="question-tabs mb-50px">
                        @if(!empty($Questions) && count($Questions) > 0 )
                            ...
                            @endif
                            <div class="row">
                                <div class="col-md-10 all_questions">
                                    <ul class="nav nav-tabs generic-tabs" id="myTab" role="tablist">
                                        ...
                                        <li class="nav-item me-2">
                                            <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab"
                                                    ...
                                                    aria-selected="true">{{__('front.All Quetions')}}</button>
                                        </li>
                                        <li class="nav-item me-2">
                                            <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab"
                                                    data-bs-target="#nav-profile" type="button" role="tab"
                                                    aria-controls="nav-profile"
                                                    aria-selected="false">{{__('front.My Questions')}}</button>
                                        </li>
                                    </ul>
                                </div>
                                <div class="col-md-2 create_question">
                                    <a href="{{route('blog_question_create')}}"
                                       class="btn btn-primary mb-2 add-quetion">{{__('front.Add New Quetion')}} </a>
                                </div>
                            </div>
                            <div class="tab-content" id="nav-tabContent">
                                            ...
                                            @forelse($Questions as $question)
                                                <div class="media media-card media--card align-items-center">
                                                    <div class="media-body">
                                                        <h5>
                                                            <a href="{{route('questions.show' , $question->id )}}"> {!! $question->details !!}</a>
                                                        </h5>
                                                        <small class="meta">
                                                            <span
                                                                class="pr-1">{{ $question->created_at->format('d/m/Y') }}</span>
                                                        </small>
                                                    </div>
                                                </div><!-- end media -->
                                            @empty
                                ...
                                <div class="tab-pane fade" id="nav-profile" role="tabpanel"
                                    ...
                                    <div class="question-main-bar best-question-bar pt-4">
                                        <div class="questions-snippet">
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Check if there's a success message element
            var successMessage = document.getElementById('success-message');

            // If success message element exists
            if (successMessage) {
                // Show the message
